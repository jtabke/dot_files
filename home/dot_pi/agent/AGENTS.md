...
- Before launching a worker, split work that spans multiple independently testable seams or is likely to exceed 15 minutes. Give each worker one coherent mutation checkpoint and focused validation.
- After a stable diff, launch 2–3 distinct read-only review or validation angles in one `runs.all` wave. Do not pass acceptance policies to read-only agents.
...
- After a worker timeout or incomplete handoff, inspect once and send only a smaller checkpoint to a resumable worker or one labeled fallback. Do not relaunch the full task.
