...
- Use one async `workflowScript` for coordinated model work and await or return every child operation. Do not wait, poll, or inspect status merely to observe progress. Bound launches and external commands with explicit timeouts. After one timeout, inspect once and resume only a smaller checkpoint.
